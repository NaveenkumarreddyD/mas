...
echo "Checking no old instance name remains..."
if grep -Rni "drroc4-mas" mas bootstrap docs vault scripts 2>/dev/null; then
  echo "ERROR: found old drroc4-mas string"
  fail=1
fi

echo "Checking no active Db2U config files..."
if [[ -f "mas/drroc4/drmasapp/ibm-db2u.yaml" || -f "mas/drroc4/drmasapp/ibm-db2u-databases.yaml" ]]; then
  echo "ERROR: active Db2U files found"
  fail=1
fi

echo "Checking region id..."
if ! grep -R "id: lac1" mas/drroc4/ibm-mas-cluster-base.yaml mas/drroc4/drmasapp/ibm-mas-instance-base.yaml >/dev/null; then
  echo "ERROR: expected region id lac1 in base files"
  fail=1
fi

echo "Checking AWS references in active MAS config..."
if grep -RniE "arn:aws|secretsmanager|us-east-1|amazon|route53|cloudwatch|efs|ecr|rds|docdb|rosa|sts|iam" mas/drroc4 2>/dev/null; then
